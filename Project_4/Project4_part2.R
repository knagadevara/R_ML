...
#### Extra Testing Stack
Stack_training_Set$TEST_STCK_left = predTrain_StackTrain 
FGLM = glm(left ~ . -time_spend_company_3 -number_project_4 -time_spend_company_2  , Stack_training_Set , family = 'binomial')
falscore = predict(FGLM,newdata = Stack_training_Set , type = 'response')
fauc_score=auc(roc(Stack_training_Set$left , falscore))
fauc_score
fitted.results_st <- ifelse(falscore > 0.409,1,0)
Stack_training_Set$FGLM_DATA = fitted.results_st

## Predicting the data in Testing set and saving them in testing dataframe
Stack_testME$TEST_STCK_left = predTest_Stack
val_score_test = predict(FGLM,newdata = Stack_testME , type = 'response')
StKfitted_test_results <- ifelse(val_score_test > 0.409,1,0)
Stack_testME$FGLM_DATA = StKfitted_test_results

### Random Forest
rf_stack_trainM = randomForest(left ~ . -time_spend_company_3 -number_project_4 -time_spend_company_2  , Stack_training_Set , stepFactor=0.5, improve=1e-5, ntree=500, mtry=10 )
## Predicting
Spred1_train = predict(rf_stack_trainM,Stack_training_Set)
confusionMatrix(Spred1_train, Stack_training_Set$left)
plot(rf_model_train1)
# ### Saving the obtained predicted column in a new training data set
# RF_MOD1 = Loggy_Train
# RF_MOD1$RF1_left = pred1_train
## Predicting the data in Testing set and saving them in testing dataframe
Spred1_test = predict(rf_stack_trainM,Stack_testME)

write.csv(Spred1_test , file = "SaiKarthik_NagadevaraP4P2.csv" , row.names = F)